...
?>
  <body>
	<div class="container-fluid">
		...
		<div class="top2">	
			<font style="color: white;"><p style="float: left;margin-left: 10px; margin-top: 9px;"><?php echo $objResult["email"];?></p></font>
...
<?php
        ...
        $objCon = mysqli_connect($serverName,$userName,$userPassword,$dbName);
        mysqli_set_charset($objCon, "utf8");
        $strSQL = "SELECT * FROM addressuser WHERE UserID = '".$_SESSION['UserID']."'";
        $objQuery = mysqli_query($objCon,$strSQL);
        ?>
        <?php
		...
		<div align="center" style="width: 400px" class="panel panel-default">
            ชื่อ : <?php echo $objResult["name"];?>
            <?php echo $objResult["lastname"];?><br>
            บ้านเลขที่ :  <?php echo $objResult["address"];?><br>
            ที่อยู่ : <?php echo $objResult["district"];?>
            <?php echo $objResult["amphur"];?>
            <?php echo $objResult["province"];?>
            <?php echo $objResult["zipcode"];?><br>
            เบอร์โทรศัพท์ : <?php echo $objResult["tel"];?><br><br>
            <small><a href="save_addressout.php?number=<?php echo $objResult["number"];?>">ส่งที่อยู่นี้</a></small> | <small><a href="edit_addressuser.php?number=<?php echo $objResult["number"];?>">แก้ไข</a></small> | <small><a href="delete_addressuser.php?number=<?php echo $objResult["number"];?>">ลบ</a></small>
            </div>
            <?php
  					}
					?>
